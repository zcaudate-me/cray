  ...
  (let [intersect-result (aabb-intersect-ray box ray -10000.0 10000.0)]
    (if (nil? intersect-result)
      ...
      (let [[tnear tfar] intersect-result
            entry (if (> tnear epsilon)
                    (vec-add (ray :origin) (vec-scale tnear (ray :direction))) ; a ray with external origin
                    (ray :origin)) ; a ray with internal origin
            exit (vec-add (ray :origin) (vec-scale tfar (ray :direction)))
            axis-entry (split-axis entry)
            axis-exit (split-axis exit)
            entry-in-left (<= axis-entry split-pos)
            exit-in-left (<= axis-exit split-pos)
    ...
    (col-if (< trace-depth ((scene :settings) :max-trace-depth))
      (let [hit-info (closest-object-tree ray (scene :scene-tree))]
       ...
       (let [translated-light-vecs (map #(vec-sub % (hit-info :hit-point)) light-vecs)
         ...
         (reduce #(%2 shade-info %1) (settings :ambient-light) ((shade-info :obj) :shaders))))))))
...
(defn trace-band 
  ...
  (let [{:keys [x-res y-res shadow-samples soft-shadow-width look-at viewpoint camera-up
        ...
        top-left (virtual-screen 0)
        dx (vec-scale (/ 1.0 x-res) (vec-sub (virtual-screen 1) top-left))
        dy (vec-scale (/ 1.0 y-res) (vec-sub (virtual-screen 3) top-left))
...
(defn render-bands 
  ...
  [scene]
  (let [{:keys [x-res y-res image-bands]} (scene :settings)
        image (make-array Integer/TYPE (* x-res y-res 3))]
    (doall (pmap #(trace-band image scene %) (range image-bands)))
    image))
...
(defn trace  
  ...
  (render-bands
    (array-map
      ...
      :scene-tree (build-scene-tree scene-data settings)
      :settings settings))
  scene)

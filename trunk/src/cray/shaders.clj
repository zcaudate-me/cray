...
(defn get-bump [{:keys [obj]} -info]
  ((obj :bumpmap) (calc-uv shade-info 1.0 1.0)))

(defn get-bumped-normal
  [{:keys [obj]]
    (let [normal (get-normal shade-info)]
      (if (obj :bumpmap)
        (normalize (vec-add (get-bump shade-info) normal))
        normal)))
...
(defn texture-shader 
  ...
  ([texture u-scale v-scale amount]
    (fn [shade-info colour-acc]
      (vec-add colour-acc
        (vec-scale amount
          (texture (calc-uv shade-info u-scale v-scale)))))))

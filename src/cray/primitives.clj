...
(defmethod get-normal :triangle get-normal-tri [{:keys [obj uv]}] 
	(let [n0 (obj :normal0)
				...
				n2 (obj :normal2)
				[u v] uv]
		(normalize (vec-add n0
												(vec-scale u (vec-sub n1 n0)) 
												(vec-scale v (vec-sub n2 n0))))))
...
(defmethod calc-uv :sphere [hit-info u-scale v-scale] 
  (let [vp (get-normal hit-info)
				sphere-up ((hit-info :obj) :up)
				sphere-left ((hit-info :obj) :left)
...
(defmethod calc-uv :plane [{:keys [obj, hit-point]} u-scale v-scale] 
...
(defmethod calc-uv :triangle [{:keys [uv]} u-scale v-scale]
  (let [[u v] uv]
    [(* u u-scale) (* v v-scale)]))
